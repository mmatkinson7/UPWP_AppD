...
setwd("~/Documents/WorkStuff/UPWP")

apis <- listCensusApis()
View(apis)

#load ACS 2018 5YR SF Appendices
#The Appendices list the tables, table description, table universe
ACS_2018_SF_5YR_Appendices <- read_excel("ACS_2018_SF_5YR_Appendices.xls")

#load ACS 2018 5YR Table Shells
#The Table Shells list and describe all the columns in each table.
ACS2018_Table_Shells <- read_excel("ACS2018_Table_Shells.xlsx")
...
UPWP_2018df <- data.frame(DTabs$GEO_ID)
UPWP_2018df <- rename(UPWP_2018df, replace =c("GEO_ID"="DTabs.GEO_ID"))
UPWP_2018df$NAME_FULL <- DTabs$NAME
UPWP_2018df$Total_Population <- DTabs$B02001_001E
UPWP_2018df$Minority_Perc <- ((DTabs$B02001_001E - DTabs$B02001_002E)/DTabs$B02001_001E)*100
UPWP_2018df$LEP_Perc <-  ((DTabs$C16001_005E + DTabs$C16001_008E + DTabs$C16001_011E + DTabs$C16001_014E + 
...
UPWP_2018df$Low_Income_Perc <- ((DTabs$C17002_002E + DTabs$C17002_003E + DTabs$C17002_004E + DTabs$C17002_005E + 
                                   DTabs$C17002_006E + DTabs$C17002_007E)/ DTabs$B02001_001E)*100
UPWP_2018df$Median_Income <- DTabs$B19013_001E

#split the NAME column to just be the town names
nametemp <- str_split_fixed(UPWP_2018df$NAME, " town",2)
nametemp <- str_split_fixed(nametemp[,1], " city",2)
nametemp <- str_split_fixed(nametemp[,1], " Town",2)
nametemp <- nametemp[,1]
UPWP_2018df$NAME <- nametemp
...
UPWP_SUB <- UPWP_2018df %>% filter(UPWP_2018df$NAME %in% BRMPO)

#load original table that we are updating
UPWP_OLD_TABLE <- read_excel("2019-04-23 Table D-1 UPW SJ 2a CLIENT REVIEW.xlsx")
...
#rename this column before replacing data because now pop based instead of households
...
MAPC <- data.frame(UPWP_2018df$NAME)
...
ICC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% ICC),])
ICC_Tab$SubRegion <- "ICC"
ICC_Tab$UPWP_2018df.NAME <- as.character(ICC_Tab$UPWP_2018df.NAME)

SSC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% SSC),])
SSC_Tab$SubRegion <- "SSC"
SSC_Tab$UPWP_2018df.NAME <- as.character(SSC_Tab$UPWP_2018df.NAME)

TRIC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% TRIC),])
TRIC_Tab$SubRegion <- "TRIC"
TRIC_Tab$UPWP_2018df.NAME <- as.character(TRIC_Tab$UPWP_2018df.NAME)

SWAP_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% SWAP),])
SWAP_Tab$SubRegion <- "SWAP"
SWAP_Tab$UPWP_2018df.NAME <- as.character(SWAP_Tab$UPWP_2018df.NAME)

MWRC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% MWRC),])
MWRC_Tab$SubRegion <- "MWRC"
MWRC_Tab$UPWP_2018df.NAME <- as.character(MWRC_Tab$UPWP_2018df.NAME)

MAGIC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% MAGIC),])
MAGIC_Tab$SubRegion <- "MAGIC"
MAGIC_Tab$UPWP_2018df.NAME <- as.character(MAGIC_Tab$UPWP_2018df.NAME)

NSPC_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% NSPC),])
NSPC_Tab$SubRegion <- "NSPC"
NSPC_Tab$UPWP_2018df.NAME <- as.character(NSPC_Tab$UPWP_2018df.NAME)

NSTF_Tab <- data.frame(MAPC[which(MAPC$UPWP_2018df.NAME %in% NSTF),])
NSTF_Tab$SubRegion <- "NSTF"
NSTF_Tab$UPWP_2018df.NAME <- as.character(NSTF_Tab$UPWP_2018df.NAME)
...
write_xlsx(UPWP_SUB_Towns, "UPWP2020.xlsx")
write_xlsx(UPWP_JOIN, "UPWP_EXTRA.xlsx")
...
indices <- c(which(MAPC$UPWP_2018df.NAME %in% ICC))
for (i in indices){
  #update subregion column
  MAPC[i, 7] <- subReg_Names[index]
}



#Get Whole Table
df2 <- getCensus(name = "acs/acs5", vintage = 2018,key = key,
                 vars = c("NAME", "group(B19013)"),
                 region = "county subdivision:*", regionin = "state:25")
#region= "county:*", "tract:*", "block:*"
head(df2)

#get metadata for the variables in a group
metagroup_B17020 <- listCensusMetadata(name = "acs/acs5",
                                       vintage = 2017,
                                       type = "variables",
                                       group = "B01002")
head(metagroup_B17020)



#subject tables require a different api than the detailed tables
S1901df <- getCensus(name = "acs/acs5/subject", vintage = 2018, key = key,
                     vars = c("GEO_ID", "S1901_C01_001E","S1901_C01_002E","S1901_C01_003E","S1901_C01_004E",
                              "S1901_C01_005E","S1901_C01_006E"), region = "county subdivision:*", regionin = "state:25")
#merge the tables 
UPWP_2018df <- merge(DetailTables, S1901df, by= c("GEO_ID", "county", "state", "county_subdivision"))
